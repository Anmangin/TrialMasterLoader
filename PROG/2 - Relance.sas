...
************************************************************************************************************************************************************************
------------------ ne rien toucher apres ici ---------------------
Description du dossier :
dossier DB : contient les donnees de l'etude exporte
dossier Status : contients les statuts eporte de l'etude
dossier IMP : contients les elements a importer.
...
/*    creation d'une table nomme nomtable dans la work avec la liste de table present dans la libname appelle */
...
%macro creation_DCR;
 %if %sysfunc(libref(NOTE)) NE 0 %then %do;
    ...
    %end;
	%else %do;
%getnomtable(NOTE);	 /* creation de nomtable, la liste des tables dans NOTE */
...
if finalf then call symputx('nbtable',_N_); /* creation de la macro variable &nbtable le nombre de table dans la librairie note */
run;


%do i=1 %to &nbtable; /* on cree une boucle pour gerer toutes les tables  1 a  1 */
%put travail sur la table &&memname&i;



/* evaluation des formats dans la table charge */

proc contents noprint data=db.&&memname&i out=temp_format;run;
 	%if &i=1 %then %do;data format_db;set temp_format;run;%end;  /* chargement dans format_db a  la premiere iteration */
%if &i>1 %then %do;data format_db;set format_db temp_format;run;%end;  /* chargement dans format_db dans les autres iterations  dans eliminer les informations deja  charge */
...
%if &i=1 %then %do;data DCR;set tempDCR;run;%end;  /* chargement dans DCR a  la premiere iteration */
%if &i>1 %then %do;data DCR;set DCR tempDCR;run;%end;  /* chargement dans DCR dans les autres iterations  dans eliminer les informations deja  charge */
%end;
...
/* chargement des formats associe dans la table DCR */
...
/* par defaut, DCR contient les codes sans les formats, ici on cree la variable responseTXT qui contient la decode pour les categories et les dates */
...
%if %sysfunc(libref(STATUS)) NE 0 %then %do;
	...
	%getnomtable(STATUS);	 /* creation de nomtable, la liste des tables dans NOTE */

  data _null_;
set nomtable end=finalf;
call symputx('memname'||left(_N_),memname);	/* chargement du nom de la table dans &memname1 / &memname2 / etc... pour la boucle */
if finalf then call symput('nbtable',_N_); /* creation de la macro variable &nbtable le nombre de table dans la librairie note */
run;
...
%end;
  ...
  %suppr(temp_FormStatus);
   ...
   /* Creation de VDAT dans un format adapte */
...
/* on creecris zRespDat mais avec un bon format datetime exploitable */
data FormStatus;
set FormStatus;
...
run;
 ...
 /* Securite on verifie la presence de DCR avant d'ajouter le nombre de queries par fiche */
 %getnomtable(WORK);
 data nomtable; set nomtable; where memname="DCR";run;
 proc sql noprint; select count(*) into: pres_dcr from nomtable;quit;
 %suppr(nomtable);
 %put nombre de table DCR present (1 attendu) : &pres_dcr ;
...
/* API decrypt : m=17 -> aller dans l'onglet Browse
  ...
  &otid=8&said=10 -> je sais pas mais ca marche
   tgn-> faire remplir le nom de l'etude automatiquement
